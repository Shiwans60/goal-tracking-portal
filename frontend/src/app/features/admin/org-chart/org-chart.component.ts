...
@Component({
          ...
          <mat-card-content>
            @for (node of searchResults(); track node.id) {
              <div class="search-result">
                <div class="node-avatar" [class]="avatarClass(node.role)">
                ...
                <div class="node-info">
                  ...
                  @if (node.department) {
                    <div class="node-sub">{{ node.department }}</div>
                  }
                </div>
                <div class="node-meta">
                  <mat-chip [class]="chipClass(node.role)">{{ friendlyRole(node.role) }}</mat-chip>
                  @if (node.managerName) {
                    <div class="reports-to">Reports to: {{ node.managerName }}</div>
                  }
                  @if (node.directReportCount > 0) {
                    <div class="direct-count">{{ node.directReportCount }} direct reports</div>
                  }
                </div>
              </div>
              <mat-divider />
      ...
      <div class="tree-node" [style.padding-left.px]="node.depth * 32">
        <div class="node-row" [class.has-children]="node.children.length > 0">
          ...
          <div class="node-info">
            ...
            @if (node.department) {
              <span class="node-sub dept">
                <mat-icon class="dept-icon">domain</mat-icon>{{ node.department }}
              </span>
            }
          </div>

          <!-- Role chip + report count -->
  ...
  styles: [`
    ...
    .dept-icon { font-size: 12px; height: 12px; width: 12px; line-height: 12px; }
...
export class OrgChartComponent implements OnInit {
  ...
  /** Tree roots built from flat list */
  ...
  avatarClass(role: string): string {
    return ({
      ROLE_EMPLOYEE: 'node-avatar avatar-employee',
      ROLE_MANAGER:  'node-avatar avatar-manager',
      ROLE_ADMIN:    'node-avatar avatar-admin',
    })[role] ?? 'node-avatar avatar-employee';
  }

  chipClass(role: string): string {
    return ({
      ROLE_EMPLOYEE: 'chip-employee',
      ROLE_MANAGER:  'chip-manager',
      ROLE_ADMIN:    'chip-admin',
    })[role] ?? '';
  }
}